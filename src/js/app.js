...
function slider() {
    var slideRadios = [
        document.querySelector('#slide_radio_1'), 
        document.querySelector('#slide_radio_2'), 
        document.querySelector('#slide_radio_3')
    ]; 
    for(let i = 0; i < slideRadios.length; i++) {
        if(slideRadios[slideRadios.length - 1].checked) { slideRadios[0].checked = true; break;}
        if(slideRadios[i].checked) { slideRadios[++i].checked = true;}
    }
}
